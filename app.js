// Influencer Empire — Dark v2 with robust navbar (click + touch) and error-guarded router
(function(){
  ...
  const SAVE_KEY = "influencer_empire_dark_v2";
  ...
  function recalc(){
    ...
    state.sources.forEach(s=>{ const base=s.baseRate*s.level; s.rate = base * state.multipliers.passive * state.multipliers.all * boostMult; });
  }

  function offlineEarnings(){
    const now = Date.now(), diff = Math.max(0, now - state.lastTs), maxMs = 8*60*60*1000;
  ...
  function tick(){
    ...
    if (state.boost.active && now >= state.boost.until){ state.boost.active=false; state.boost.mult=1; recalc(); }
    ui.update(); save();
  }
  ...
  function renderSources(){
    ...
    state.sources.forEach((s, idx)=>{
      ...
      btn.addEventListener("click", ()=> buySourceLevel(idx), {passive:true});
  ...
  function renderShop(tab="sprzet"){
    els(".tab").forEach(t=>{
      ...
      t.ontouchstart = ()=> renderShop(t.dataset.tab);
    });
    ...
    state.upgrades[tab].forEach((u, idx)=>{
      ...
      if (u.type.startsWith("ad_")){
        ...
        b.addEventListener("click", ()=> buyUpgrade(tab, idx), {passive:true});
  ...
  function showAd(seconds, onDone){
    ...
    close.addEventListener("touchstart", handler, {once:true, passive:true});
  }

  function toast(msg){ if (tg && tg.showPopup){ tg.showPopup({title:"Info", message: msg, buttons:[{type:"ok"}]}); } else { console.log("[INFO]", msg); } }

  function dailyMissions(){
    const today = new Date().toDateString(), key="missions_day", last=localStorage.getItem(key);
    if (last !== today){
      const m=[
      ...
      localStorage.setItem("missions", JSON.stringify(m)); localStorage.setItem(key, today);
    }
  }
  function renderMissions(){
    ...
    list.forEach((m, idx)=>{
      const li = document.createElement("li"); const done = m.progress>=m.goal;
    ...
    ul.querySelectorAll("button[data-claim]").forEach(b=>{
      b.addEventListener("click", ()=>{
        const i = parseInt(b.dataset.claim); const arr=JSON.parse(localStorage.getItem("missions")||"[]"); const mm = arr[i];
        if (mm && mm.progress>=mm.goal && !mm.claimed){
          state.followers += mm.reward; state.total += mm.reward; mm.claimed = true;
          localStorage.setItem("missions", JSON.stringify(arr)); renderMissions(); ui.update(); toast("Nagroda odebrana!");
        }
      }, {passive:true});
    });
  }
  function incMission(id, amount){
    ...
    if (m && !m.claimed){ m.progress += amount; localStorage.setItem("missions", JSON.stringify(arr)); renderMissions(); }
  }
  ...
  function bind(){
    el("#btnClick").addEventListener("click", ()=>{ clickGain(); incMission("click500",1); }, {passive:true});
    el("#btnAdCollect").addEventListener("click", ()=> showAd(5, ()=> instantCollect(60*60)), {passive:true});
    el("#btnAdBoost").addEventListener("click", ()=> showAd(5, ()=> activateBoost(2,5*60)), {passive:true});
    el("#btnAdChest").addEventListener("click", ()=>{
      const left = state.chest.next - Date.now();
      if (left>0) { toast("Jeszcze w cooldownie!"); return; }
      showAd(5, ()=>{
        const isStars = Math.random()<0.15, amount = isStars?5:1000+Math.random()*4000;
        state.followers += isStars?0:amount; state.total += isStars?0:amount;
    ...
    els(".nav").forEach(n=>{
      ...
      const go = ()=> show(targetView);
      n.addEventListener("click", go, {passive:true});
      n.addEventListener("touchstart", go, {passive:true});
    });
    el("#settingsBtn").addEventListener("click", ()=> show("profile"), {passive:true});

    el("#saveNick").addEventListener("click", ()=>{
      const v = (el("#nickInput").value||"").trim().replace(/\s+/g,"_");
      if (v){ state.nick = v; ui.update(); save(); toast("Zapisano nick"); }
    }, {passive:true});
  ...
  function init(){
    load(); recalc(); offlineEarnings(); bind(); ui.update(); seedRanking(); show("start");
    setInterval(tick, 1000);
    const nickInput = el("#nickInput"); if (nickInput) nickInput.value = state.nick;
    renderShop("sprzet");
  }

  window.addEventListener("load", ()=> safe(init));
})();